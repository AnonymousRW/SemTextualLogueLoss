...
def train(model, iterator, optimizer, criterion, clip, epoch, total, trg_eos_token,SRC,TRG,embed_model):
    ...
    for i, batch in enumerate(iterator):
        ...
        loss=0
        for cur_trg,cur_new_trg,cur_new_src,cur_output in zip(trg,new_trg,new_src,output):
            ...
            cosine_similarity_context = np.dot(output_embedding,src_embedding)/(norm(output_embedding)*norm(src_embedding))

            average_cosine= (0.6*cosine_similarity+0.4*cosine_similarity_context)
            loss += (1-(average_cosine))*criterion(cur_output, cur_trg)
